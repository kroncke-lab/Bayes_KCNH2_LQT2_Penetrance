...
con = dbConnect(SQLite(),
                dbname="../VariantKCNH2-v4_w_paris_japan_and_italycohort.db")
...
# Read in cohort data
france.data <- read.csv("/Users/KRONCKE/Box Sync/Kroncke_Lab/kcnh2/VariantDatabase/Network_KCNH2_variants/French_cohort/France_cohort.csv")
italy.data <- read.csv("/Users/KRONCKE/Box Sync/Kroncke_Lab/kcnh2/VariantDatabase/Network_KCNH2_variants/Italy_cohort/Italy_cohort.csv")
japan.data <- read.csv("/Users/KRONCKE/Box Sync/Kroncke_Lab/kcnh2/VariantDatabase/Network_KCNH2_variants/Japan_cohort/Japan_cohort-v2.csv")
...
cb<-read.table("C:/Users/KRONCKE/Box Sync/Kroncke_Lab/kcnh2/VariantDatabase/PredictiveFeatureServers/cardiacboost_arm_all_possible_mutations-adj.txt", sep = "\t", header = TRUE)
cb<-cb[cb$gene=="KCNH2",]
...
pssm<-read.table("C:/Users/KRONCKE/Box Sync/Kroncke_Lab/kcnh2/VariantDatabase/PredictiveFeatureServers/DY48SUFA014-PSSM_Scoremat.asn_matrix.txt", sep = "\t", header = TRUE)
pssm_melt<-melt(data = pssm, id.vars = c("resnum", "nativeAA"), measure.vars = AAs, variable.name = "mutAA", value.name = "blast_pssm")
pph2<-read.csv("pph2-adj.csv", strip.white = TRUE)
pph2 <- pph2[,c("nativeAA", "resnum", "mutAA", "pph2_prob")]
prov<-read.csv("provean_20190522.csv", strip.white = TRUE)
prov <- prov[,c("var","provean_score")]
pam_aa <- d[,c("var", "resnum", "nativeAA", "pamscore","aasimilaritymat", "mut_type")]
revel <- read.csv("../PredictiveFeatureServers/revel_score_adj.csv", header = TRUE)

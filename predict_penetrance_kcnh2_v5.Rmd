---
title: "Predict Variant Penetrance Using Structure, Function, and *In Silico* Features (*KCNH2* and LQT2)"
...
setwd("/Users/KRONCKE/Box Sync/Kroncke_Lab/kcnh2/VariantDatabase/penetrancePrediction/")
...
for(rec in 1:nrow(mut_type)){
  if (!is.na(mut_type[rec, "resnum"])) {
    h2.covariates[h2.covariates$var == mut_type[rec, "var"], 
                  c("lqt2_dist", "lqt2_dist_weight", "resnum")] <- c(funcdist(mut_type[rec, "resnum"], mut_type[rec, "var"], d, h2dist, "penetrance_lqt2", "sigmoid", 7), mut_type[rec, "resnum"])
  }
}
...
plot(h2.covariates$resnum, h2.covariates$lqt2_dist, xlab ="Residue", ylab = "LQT2 Penetrance Density")
xrange <- seq(min(fit$x), max(fit$x), length.out = 100)
ps <- predict(fit, xrange, se=T)
lines(xrange, ps$fit*1, lwd=5)
lines(xrange, (ps$fit+1.96*ps$se.fit)*1, lty=2, lwd=4)
...
##  LQT2 penetrance in Cohort Data
...
BootsCoverage <- function(var,n=100,N=1000,true){
  ...
  if(true=="empirical") true="lqt2_penetranceBayesian_initial"
  if(true=="observed") true="penetrance_lqt2"
...
# Evaluation: Pearson R^2 and Spearman Rho Against Empirical Posterior from Cohort

```{r}
# Select covariates from isoform "A" in cohort dataset
cohort.data <- cohort.data[!is.na(cohort.data$mut_type) & !is.na(cohort.data$total_carriers) & cohort.data$isoform == "A",]
...
xList=c('hm_ssPeak','hm_tailPeak','hm_vhalfact','hm_vhalfinact','hm_recovfrominact', 'hm_taudeact_fast', 
        'ht_ssPeak','ht_tailPeak','ht_vhalfact','ht_vhalfinact','ht_recovfrominact', 'ht_taudeact_fast',
        'pph2_prob', 'provean_score', "blast_pssm",
        'pamscore', 'aasimilaritymat', "lqt2_dist", 'lqt2_penetranceEstimate_EM', "revel_score")
resultTable<-calcAllPvals(yList, xList, 1000, 'weight', tmp)

foo <- boot(tmp, function(data,indices)
  weightedCorr(tmp$lqt2_penetranceEstimate_EM[indices],tmp$penetrance_lqt2[indices], method="pearson", weights = tmp$weight[indices])^2, R=1000)
...
modfunc7 <- glm(lqt2_patho~lqt2_penetranceEstimate_EM, data = fglm, family = 'binomial')
...
for (co in cutoffs){
  ...
  EM[i]<-glm.mod(fglm,"lqt2_penetranceEstimate_EM")
  }
}
...
## ROCs of observed cohort LQT2 penetrance with 0.5 as cutoff, single observation variants. 
...
## ROC of observed literature LQT2 penetrance with 0.5 as cutoff.

## AUCs from ROCs observed literature LQT2 penetrance at multiple cutoffs

# Combine literature and cohort data

## Part 3 Forest plots (Needs revisions!)
